...
def coarsen(A, levels, self_connections=False):
    graphs, parents = metis(A, levels)  # Assuming metis is a function that returns graphs and parents
    perm = compute_perm(parents)
    
    # Ensure perm length matches the number of rows in the original data
    if len(perm) < A.shape[0]:
        perm.extend(range(len(perm), A.shape[0]))
...
def perm_data(data, perm):
    """
    Permute the data according to the permutation indices.
    """
    M = data.shape[0]
    Mnew = len(perm)
    ...
    assert Mnew >= M, f"Assertion failed: Mnew ({Mnew}) is not greater than or equal to M ({M})"
    
    permuted_data = data[perm, :]
    
    # Debug statement to check permuted data shape
